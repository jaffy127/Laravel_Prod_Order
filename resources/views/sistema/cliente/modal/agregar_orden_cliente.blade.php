	...
	<div class="modal-content ">
	    ...
	    <form id="addEditFormOrden" method="post">
			<div class="form-row">
				<div class="form-group col-md-6">
					...
					<select id="id_cliente" name="id_cliente" class="form-control" {!! (!empty($result_orden_by_id) && !empty($result_orden_by_id->id_cliente) || !empty($result_cliente_id)) ? "disabled" : "" !!}>
						<option selected>Selecciona...</option>
						@if (!empty($result_orden_by_id) && !empty($result_orden_by_id->id_orden))
							@if (!empty($result_cliente) && !$result_cliente->isEmpty())						
								@foreach ($result_cliente as $cliente)
									<option value="{{ $cliente->id_cliente }}" 
										{!! ((($result_orden_by_id->id_cliente == $cliente->id_cliente)  || !empty($result_cliente_id)) ? "selected=\"selected\"" : "") !!}>
						...
						@else
							@if (!empty($result_cliente) && !$result_cliente->isEmpty()))						
								@foreach ($result_cliente as $cliente)
									<option value="{{ $cliente->id_cliente }}" 
										{!! ((($cliente->id_cliente == $result_cliente_id)) ? "selected=\"selected\"" : "") !!}>
										{{ $cliente->nombre_cliente }}
									</option>
								@endforeach
							@endif
	  ...
	  <div class="modal-footer" id="addEditFormOrden_footer">
		...
		@if (!@empty ($result_orden_by_id) && !empty($result_orden_by_id->id_orden))
			...
			<button type="button" class="btn btn-primary" id="putEditOrden" data-flag-modal="by_cliente">Guardar Cambios</button>
		@else
			<button type="button" class="btn btn-primary" id="postAddOrden" data-flag-modal="by_cliente">Guardar</button>
		@endif	    
	  </div>
	</div>
</div>